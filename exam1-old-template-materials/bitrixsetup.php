	...
	do
	{
		...
		else
		{
			...
			if (strlen($strLocationUrl)>0)
			{
				...
				{
					$strRealUrl = dirname($lasturl) . "bitrixsetup.php/" .$redirection;
				}
				else
				{
...
class CArchiver
{
			...
			if ($v_extract_file && !$v_listing)
			{
				...
				if ($v_extract_file)
				{
					...
					else
					{
						...
						{
							$this->_arErrors[] = array("ERR_CREATE_FILE", LoaderGetMessage('NO_PERMS') . ' bitrixsetup.php' . $v_header['filename']);
							return false;
						}
						else
						{
	...
	function _readHeader($v_binary_data, &$v_header)
	{
		...
		$chars = count_chars(substr($v_binary_data, 0, 148) . '        bitrixsetup.php' .substr($v_binary_data,156,356));
		$v_checksum = 0;
		...
		$v_header['filename'] = trim($v_data['prefix'], "\x00") . "bitrixsetup.php/" .trim($v_data['filename'], "\x00");
